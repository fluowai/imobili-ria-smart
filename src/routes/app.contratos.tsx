...
export const Route = createFileRoute("/app/contratos")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Templates de contrato, assinatura digital e controle de vigência.",
      },
    ],
  }),
...
function ContratosPage() {
  ...
  const filtrados = useMemo(
    () =>
      contratos.filter((c: any) => {
        ...
        if (
          busca &&
          !`${c.titulo} ${c.id} ${c.partes.join(" ")}`.toLowerCase().includes(busca.toLowerCase())
        )
          return false;
        return true;
      }),
  ...
  const kpis = useMemo(
    () => ({
      ...
      valorAtivo: contratos
        .filter((c: any) => c.status === "ativo")
        .reduce((a: number, c: any) => a + c.valor, 0),
    }),
    [contratos],
  );
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          <div className="flex flex-col gap-3 rounded-xl border border-border bg-card p-4 md:flex-row md:items-center">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Buscar contratos por título, ID ou parte..."
                value={busca}
            ...
            <select
              value={statusSel}
              onChange={(e) => setStatusSel(e.target.value as ContratoStatus | "todos")}
                ...
                <article key={c.id} className="rounded-xl border border-border bg-card p-4">
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-3 text-xs">
                    <div>
                      ...
                      <p className="mt-0.5 font-medium">{fmtBRLFull(c.valor)}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Criado</p>
                      <p className="mt-0.5 font-medium">{c.criadoEm}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Vencimento</p>
                      <p className="mt-0.5 font-medium">{c.vencimento ?? "—"}</p>
                    </div>
                  </div>
                  ...
                  {c.totalAssinantes > 0 && (
                    <div className="mt-3">
                      <div className="flex justify-between text-[11px] text-muted-foreground">
                        ...
                        <span>
                          {c.assinadoPor}/{c.totalAssinantes}
                        </span>
                      </div>
            ...
            {filtrados.length === 0 && (
              <div className="rounded-xl border border-dashed border-border p-12 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Nenhum contrato encontrado. Crie um novo.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="rounded-xl border border-border bg-card p-5 h-fit">
          ...
          <ul className="mt-4 space-y-3">
            {templates.map((t) => (
              <li
                key={t.nome}
                className="rounded-lg border border-border p-3 hover:bg-muted/40 cursor-pointer"
              >
