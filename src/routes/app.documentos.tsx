  ...
  head: () => ({
    meta: [
      { title: "Documentos (GED) — ImobiOS" },
      {
        name: "description",
        content:
          "Gestão eletrônica de documentos com versionamento, OCR e compartilhamento seguro.",
      },
    ],
  }),
  component: GEDPage,
  ...
  const filtrados = useMemo(
    () =>
      documentos.filter((d) => {
        ...
        if (busca && !`${d.nome} ${d.pasta}`.toLowerCase().includes(busca.toLowerCase()))
          return false;
        return true;
      }),
    [busca, pasta],
            ...
            <li>
              <button
                ...
                className={cn(
                  ...
                  pasta === null && "bg-muted font-medium",
                )}
              >
            ...
            {pastasGED.map((p) => (
              <li key={p.id}>
                <button
                  ...
                  className={cn(
                    "flex w-full items-center justify-between rounded-md px-2 py-1.5 hover:bg-muted",
                    pasta === p.nome && "bg-muted font-medium",
                  )}
                >
                  <span className="flex items-center gap-2">
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-xl border border-border bg-card p-3">
            <div className="relative">
              ...
              <Input
                placeholder="Buscar por nome ou pasta (OCR ativo)..."
                value={busca}
                    ...
                    <tr key={d.id} className="border-t border-border hover:bg-muted/30">
                      ...
                      <td className="px-4 py-2">
                        <div className="flex gap-1">
                          ...
                          <Button size="sm" variant="ghost" className="h-7 w-7 p-0">
                            <Share2
                              className={cn("h-3.5 w-3.5", d.compartilhado && "text-primary")}
                            />
                          </Button>
                        </div>
                      </td>
