...
export const Route = createFileRoute("/app/urbano/chaves")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Rastreamento de chaves: quem retirou, quando, cofre, atrasos." },
    ],
  }),
  component: ChavesPage,
});

const statusInfo: Record<Chave["status"], { label: string; icon: typeof CheckCircle2; className: string }> = {
  disponivel:  { label: "Disponível", icon: CheckCircle2,  className: "bg-emerald-500/15 text-emerald-700 dark:text-emerald-300" },
  emprestada:  { label: "Emprestada", icon: KeyRound,      className: "bg-blue-500/15 text-blue-700 dark:text-blue-300" },
  atrasada:    { label: "Atrasada",   icon: AlertTriangle, className: "bg-red-500/15 text-red-700 dark:text-red-300" },
};

function ChavesPage() {
  const [filtro, setFiltro] = useState<Chave["status"] | "todos">("todos");
  const filtradas = useMemo(() => chaves.filter((c) => filtro === "todos" || c.status === filtro), [filtro]);
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        actions={<Button size="sm"><Plus className="mr-2 h-4 w-4" />Registrar retirada</Button>}
      />
            ...
            <article key={c.id} className="rounded-xl border border-border bg-card p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-start gap-3">
                  <div className={cn("flex h-10 w-10 shrink-0 items-center justify-center rounded-lg", statusInfo[c.status].className)}>
                ...
                <Badge className={cn("border-none", statusInfo[c.status].className)}>{statusInfo[c.status].label}</Badge>
              </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
                ...
                <div>
                  <p className="flex items-center gap-1 uppercase tracking-wide text-muted-foreground"><MapPin className="h-3 w-3" />Local</p>
                  <p className="mt-0.5 truncate font-medium">{c.local}</p>
                </div>
                {c.comQuem && (
                  <div className="col-span-2">
                    <p className="flex items-center gap-1 uppercase tracking-wide text-muted-foreground"><User className="h-3 w-3" />Com</p>
                    <p className="mt-0.5 truncate font-medium">{c.comQuem} · <span className="font-normal text-muted-foreground">desde {c.desde}</span></p>
                  </div>
                )}
                {c.finalidade && (
                  <div className="col-span-2">
              ...
              <div className="mt-4 flex gap-2">
                {c.status === "disponivel" ? (
                  <Button size="sm" className="flex-1">Emprestar</Button>
                ) : (
                  <Button size="sm" variant="outline" className="flex-1">Devolver</Button>
                )}
...
function Kpi({ label, valor, tone }: { label: string; valor: number; tone: "emerald" | "blue" | "red" | "default" }) {
  const cls = {
