  ...
  head: () => ({
    meta: [
      { title: "Valuation CAR — ImobiOS" },
      {
        name: "description",
        content:
          "Avaliação de imóveis rurais por comparáveis, com ajuste de aptidão e valor por hectare.",
      },
    ],
  }),
...
function ValuationPage() {
  ...
  const media = useMemo(
    () => comparaveis.reduce((a, c) => a + c.valorHa, 0) / comparaveis.length,
    [],
  );
  const valorHaAjustado = useMemo(
    () => Math.round(media * (0.7 + (aptidao / 100) * 0.5)),
    [media, aptidao],
  );
        ...
        <div className="rounded-xl border border-border bg-card p-5 lg:col-span-1">
          ...
          <div className="mt-4 space-y-4 text-sm">
            ...
            <div>
              ...
              <Input
                ...
                min={1}
                value={areaHa}
                onChange={(e) => setAreaHa(Math.max(1, Number(e.target.value) || 0))}
                className="mt-1"
              />
            </div>
            <div>
          ...
          <div className="mt-6 space-y-3 rounded-lg border border-border bg-muted/30 p-4">
            ...
            <div className="border-t border-border pt-3">
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
                Valor estimado ({fmtHa(areaHa)})
              </p>
              <p className="font-display text-3xl font-semibold text-primary">
                {fmtBRLFull(valorTotal)}
              </p>
          ...
          <div className="overflow-hidden rounded-xl border border-border bg-card">
            <div className="border-b border-border px-5 py-3">
              ...
              <p className="text-xs text-muted-foreground">
                Transações usadas como referência para o R$/ha.
              </p>
            </div>
