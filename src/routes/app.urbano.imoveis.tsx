  ...
  head: () => ({
    meta: [
      { title: "Imóveis Urbanos — ImobiOS" },
      {
        name: "description",
        content:
          "Catálogo completo de imóveis urbanos com galeria, ficha técnica e publicação em portais.",
      },
    ],
  }),
...
function ImoveisUrbanosPage() {
  ...
  const imoveis = useMemo(() => {
    ...
    return query.data.map((i) => ({
      ...i,
      cover:
        i.fotos?.[0] || "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=800&q=80",
  ...
  const filtrados = useMemo(() => {
    return imoveis.filter((i: any) => {
      ...
      if (finalidade !== "todos" && i.finalidade !== finalidade && i.finalidade !== "ambos")
        return false;
      if (statusSel !== "todos" && i.status !== statusSel) return false;
      if (
        busca &&
        !`${i.titulo} ${i.codigo} ${i.bairro} ${i.cidade}`
          .toLowerCase()
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-border bg-card p-4 md:flex-row md:items-center">
        ...
        <div className="flex flex-wrap gap-2">
          <select
            value={tipo}
            onChange={(e) => setTipo(e.target.value as ImovelTipo | "todos")}
          ...
          <select
            value={finalidade}
            onChange={(e) => setFinalidade(e.target.value as ImovelFinalidade | "todos")}
            className="h-9 rounded-md border border-input bg-background px-3 text-sm"
          >
            {finalidades.map((t) => (
              <option key={t.value} value={t.value}>
          ...
          <select
            value={statusSel}
            onChange={(e) => setStatusSel(e.target.value as ImovelStatus | "todos")}
            className="h-9 rounded-md border border-input bg-background px-3 text-sm"
          >
      ...
      <p className="text-sm text-muted-foreground">
        Mostrando <span className="font-medium text-foreground">{filtrados.length}</span> de{" "}
              ...
              <div className="relative aspect-[4/3] overflow-hidden bg-muted">
                ...
                <div className="absolute left-2 top-2 flex gap-1">
                  <Badge
                    className={cn(
                      "border-none",
                      statusMap[i.status as ImovelStatus]?.className || "",
                    )}
                  >
                    {statusMap[i.status as ImovelStatus]?.label || i.status}
                  </Badge>
                  {i.destaque && (
                ...
                <div className="mt-3 border-t border-border pt-3">
                  ...
                  {i.valorLocacao > 0 && (
                    <p className="text-xs text-muted-foreground">
                      Aluguel: {fmtBRL(i.valorLocacao)}/mês
                    </p>
                  )}
                </div>
              ...
              {filtrados.map((i) => (
                <tr key={i.id} className="border-t border-border hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-right font-medium">
                    {i.valorVenda > 0 ? fmtBRL(i.valorVenda) : "—"}
                  </td>
                  <td className="px-4 py-3 text-right">
                    {i.valorLocacao > 0 ? fmtBRL(i.valorLocacao) : "—"}
                  </td>
